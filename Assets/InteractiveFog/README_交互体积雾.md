...
以下按雾体覆盖整个屏幕、每个像素都执行最大步数估算。Medium 的宽高缩放为 0.5，Low 为 0.25；“像素 × 步数”仅表示 Ray March Shader 的理论采样规模，不是实测 GPU 耗时。
...
实际采样量还会受到自适应步数、场景深度截断和浓雾提前终止影响，通常低于表中上限；Medium/Low 还包含 RT/深度降采样及全分辨率合成开销，因此不能直接按 18.3%/3.3% 推导总帧耗时。`8.03 ms → 3.53 ms（约 56%）` 是 Ray March 内部优化的历史对比数据，不代表三档完整 GPU 实测结果。
